...
class VocFogRainyDataset(torch.utils.data.Dataset):
...
def build(image_set, args):
    ...
    if image_set == 'val':
        base_dir = root / 'test'
        # --- 关键修改：直接指向 RainyImages 文件夹 ---
        img_folder = base_dir / "RainyImages"
        ann_folder = base_dir / "Annotations"
        image_set_file = base_dir / 'ImageSets' / 'Main' / 'val.txt'
    else:
        raise ValueError(f'unknown image_set {image_set}, rainy test only supports val set')

    dataset = VocFogRainyDataset(
