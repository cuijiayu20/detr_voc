...
class FoggyDrivingVOCDataset(torch.utils.data.Dataset):
...
def make_foggy_driving_transforms():
    normalize = T.Compose([
        T.ToTensor(),
...
def build(image_set, args):
    ...
    assert root.exists(), f'provided Foggy Driving VOC path {root} does not exist'

    img_folder = root / "JPEGImages"
    ann_folder = root / "Annotations"
    # 假设您会为Foggy_Driving_voc数据集创建一个ImageSets/Main/val.txt文件
    image_set_file = root / 'ImageSets' / 'Main' / 'val.txt'

    dataset = FoggyDrivingVOCDataset(
        img_folder=img_folder,
        ann_folder=ann_folder,
        transforms=make_foggy_driving_transforms(),
